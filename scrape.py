...
for site, link in latest_articles.items():
    if link != "No article found":
        ...
        markdown_filename = f"articles/md/{site}/[{datetime.now().strftime('%Y%m%d')}] {title}.md"  # Save directly to articles folder
        os.makedirs(os.path.dirname(markdown_filename), exist_ok=True)  # Ensure directory exists
        with open(markdown_filename, "w", encoding='utf-8') as f:
            f.write(markdown_content)
        ...
        output_filename = f"articles/docx/{site}/[{datetime.now().strftime('%Y%m%d')}] {title}.docx"
