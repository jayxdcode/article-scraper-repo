...
const FIND_FEATURED_IMAGE_SNIPPET = `(function(){
  // look for comment nodes containing 'FEATURED IMAGE' and attempt to find img after it
  ...
  while ((node = walker.nextNode())) {
    if (node.nodeValue && node.nodeValue.toUpperCase().includes('FEATURED IMAGE')) {
      // try siblings and nextElements to find an <img>
      let cur = node.nextSibling;
      // walk forward up to N nodes to find an img
      for (let i=0;i<10 && cur;i++) {
        if (cur.nodeType === Node.ELEMENT_NODE) {
          const img = cur.querySelector ? cur.querySelector('img') : null;
          if (img && img.src) return img.src;
          // maybe cur itself is an img
...
async function discoverLatestIndex(page) {
  ...
  // apply the exact listing logic you provided: div#opinion-v2-mh -> take first -> find a.href and oped-date
